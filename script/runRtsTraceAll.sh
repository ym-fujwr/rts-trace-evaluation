...
RTS_TRACE_DIR="$HOME_DIR/accuracy"
    ...
    if [ -d $REVISIONS_DIR/$subN ]; then
		...
		for i in {1..50}; do
			...
			if [ -d $REVISIONS_DIR/$subN/$revN ] ; then
                ...
                rm -fR $RTS_TRACE_DIR/$subN/workingDir/data/selogger/*
                rm -fR $RTS_TRACE_DIR/$subN/workingDir/data/json/*
                rm -fR $RTS_TRACE_DIR/$subN/workingDir/data/token/*
			fi
		done
	fi
    #idx=idx+1
